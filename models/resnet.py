...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        print("Define a Resnet Layer")
        self.in_planes = 16

        self.conv1 = nn.Conv2d(4, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(16)

        # to keep the trasnformer nominal lets stick to this in Detr the implementation has 2048 channels in the last layer
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2)


        # we do not need this layer
