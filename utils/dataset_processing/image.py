...
class Image:
    ...
    def offset_img(self, shift_x, shift_y):
        """ Offset the rotated image to not have all the grasps in the centre """

        image= self.img.transpose( 1, 2, 0)
        

        if (shift_x < 0):
            image = np.concatenate(
                (image[:, -shift_x:, :], np.tile(image[:, -1].reshape((-1, 1, image.shape[2])), (1, -shift_x, 1))),
                axis=1)
        else:
            image = np.concatenate((np.tile(image[:, 0].reshape((-1, 1, image.shape[2])), (1, shift_x, 1)),
                                    (image[:, :image.shape[1] - shift_x, :])), axis=1)

    
        if (shift_y < 0):
            image = np.concatenate((image[-shift_y:], np.tile(image[-1], (-shift_y, 1, 1))), axis=0)
        else:
            image = np.concatenate((np.tile(image[0], (shift_y, 1, 1)), image[:image.shape[0] - shift_y]), axis=0)
    
        self.img = image.transpose( 2, 0, 1)
...
class DepthImage(Image):
    ...
    def offset_img(self, shift_x, shift_y):
        ...
        np_depth = self.img
        if (shift_x < 0):
            np_depth = np.concatenate(
                (np_depth[:, -shift_x:], np.tile(np_depth[:, -1].reshape((-1, 1)), (1, -shift_x))), axis=1)
        else:
            np_depth = np.concatenate((np.tile(np_depth[:, 0].reshape((-1, 1)), (1, shift_x)),
                                        (np_depth[:, :np_depth.shape[1] - shift_x])), axis=1)
        if (shift_y < 0):
            np_depth = np.concatenate((np_depth[-shift_y:], np.tile(np_depth[-1], (-shift_y, 1))), axis=0)
        else:
            np_depth = np.concatenate((np.tile(np_depth[0], (shift_y, 1)), np_depth[:np_depth.shape[0] - shift_y]),
                                        axis=0)
        self.img = np_depth
