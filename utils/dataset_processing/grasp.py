...
# this what we get from the cornell dataset
class GraspRectangles:
        ...
        with open(fname) as f:
            while True:
                ...
                try:
                    ...
                    # adding logic to make sure points are in sorted order
                    # refrence https://www.pyimagesearch.com/2016/03/21/ordering-coordinates-clockwise-with-python-and-opencv/

                    xSorted = gr[np.argsort(gr[:, 0]), :]
    ...
    # adding some custom function to get bbox similar to the DETR Paper
    ...
    @property
    def box_that_fits_all_graps(self):
        """
        Find the top, left, right, bottom coordinates of the grasps , together
        """
        points = [gr.points for gr in self.grs]
        points = np.vstack( points )

        #print( points )
        return np.min( points[:, 0 ]).astype(np.int), np.min( points[:, 1 ]).astype(np.int), \
               np.max(points[:, 0]).astype(np.int), np.max(points[:, 1]).astype(np.int)

    # adding draw with changes for detr we need to have x1, y1, x2, y2 such that x1 > y1 and x2 > y2
    def draw_detr(self):
        """
        Plot all GraspRectangles as form of bounding box and angle for detr
        :return: bbox[c1, c2, h, w] , angle
        """
        target = {}
        ...
        for gr in self.grs:
            #print(gr.points)
            #if gr.points[0, 1] <= gr.points[2, 1] and gr.points[0, 0] <= gr.points[2, 0]:
            """
            x0 = gr.points[0, 1]
            x1 = gr.points[2, 1]
            ...
            boxes = np.append(boxes, [[gr.points[0, 0], gr.points[0, 1],
                                       gr.points[1, 0], gr.points[1, 1],
                                       gr.points[2, 0], gr.points[2, 1],
                                       gr.points[3, 0], gr.points[3, 1]]], axis = 0)
            angle = self.find_angle( gr.points )
            angles.append(angle)
            """
            centre = gr.center ;
            angle = gr.angle ;
            ...
            scale_factor = 224
            boxes = np.append(boxes, [[ centre[0] / scale_factor
                                        , centre[1]  / scale_factor
                                        ,
                                        width / scale_factor
                                        ,
                                        length / scale_factor
...
# 2nd way of representing a grasp, this is one used in this paper as well as in similar to dexnet
class Grasp:
    """
